<?php 
	...
	if($_GPC['op'] == 'list'){	
        if (!empty($_GPC['artsortid'])) {
	        ...
	        $info = Util::getAllDataInSingleTable('zofui_sitetemp_article',array('uniacid'=>$_W['uniacid']),$_GPC['page'],15,' `time` DESC ',true,true,'id,title,time,number,img,type,sortid',$str='and sortid IN('.$artsortid.') and title like "%'.$_GPC['title'].'%" ');
        }else{
          $info = Util::getAllDataInSingleTable('zofui_sitetemp_article',array('uniacid'=>$_W['uniacid']),$_GPC['page'],15,' `time` DESC ',true,true,'id,title,time,number,img,type,sortid',$srt='and title like "%'.$_GPC['title'].'%" ');
        }
		$list = $info[0];
		$pager = $info[1];
	}
