...
class WeixinPay
{
	...
	public function wxHttpsRequestPem( $vars,$url, $second=30,$aHeader=array()) {
		...
		curl_setopt($ch,CURLOPT_SSLCERT,ST_ROOT.'/cert/'.$_W['uniacid'].'/apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY,ST_ROOT.'/cert/'.$_W['uniacid'].'/apiclient_key.pem');
		curl_setopt($ch,CURLOPT_CAINFO,'PEM');
		curl_setopt($ch,CURLOPT_CAINFO,ST_ROOT.'/cert/'.$_W['uniacid'].'/rootca.pem');
		if( count($aHeader) >= 1 )
		{
		...
		else 
		{
			...
			curl_close($ch);
			return array('result_code'=>'FAIL','err_code_des'=>'检查是否设置了正确的退款证书');
		}
	}
