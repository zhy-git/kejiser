<?php 
	...
	if( $_GPC['op'] == 'list' ) {
		...
		$info = Util::getAllDataInSingleTable('zofui_sitetemp_video',$where,$_GPC['page'],10,' `number` DESC,`id` DESC ',false,false,' id,img,title,time,type,url,author ');
		$list = $info[0];
		...
		$info = pdo_get('zofui_sitetemp_video',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id']));
		if( !empty( $info ) ) {
		...
		Util::addOrMinusOrUpdateData('zofui_sitetemp_video',array('readed'=>1),$info['id']);
