...
//悬浮图标
Vue.component('shareicon', {
    props: ['index', 'list', 'form_transparent'],
    template: '<li id="shareicon_button_this" style="width:100%;height:auto;" class="ui-draggable"  title="点击进行修改,拖动交换位置."><div v-on:click="del_left(index)" class="deleteButton"></div><div  v-on:click="onc_banner(index,$event)" class="floaticon pr"><div class="icon_right"><img :style="\'opacity:\'+(form_transparent / 100.0)+\';width:100%px;height:auto;\'" v-for="item in list" :src="item.imgurl" alt=""></div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            vm_del_item(index);
        ...
        //选中
        onc_banner: function (index, e) {
            vm_choose_item(index, 'shareicon', false);
        },
    },
})
//会员中心列
...
Vue.component('member', {
    props: ['index', 'bg_color', 'bg_img', 'text_color', 'show_qd', 'show_jf'],
    template: '<li class="ui-draggable" title="点击进行修改,拖动交换位置."><div v-on:click="del_left(index)" class="deleteButton"></div>' +
        '<div v-on:click="onc_banner(index,$event)" class="uc_head_div" :style="' + "'background-color:'" + '+bg_color+' + "';background-image:url('" + '+bg_img+' + "');'" + '" ><img style="width: 88px;height: 35px;border-radius: 5px;position: absolute;right: 0;top: 30px;" v-if="show_qd && show_qd==1" src="public/images/member/qiandao.png"><img v-if="show_jf && show_jf==1" src="public/images/member/jifen@2x.png" style="width: 88px;height: 35px;border-radius: 5px;margin-left: 220px;position: absolute;right: 0;top: 80px;"><img src="public/images/member/noface.png" ><span :style="' + "'color:'" + '+text_color+' + "';'" + '">会员昵称</span></div><span class="bottom_space"></span></li>',
    methods: {
        //删除
        del_left: function (index) {
            vm_del_item(index);
...
Vue.component('banner', {
    props: ['topimg', ' ', 'index', 'list', 'ly_height', 'jiaodian_dots', 'bg_color', 'jiaodian_color'],
...
Vue.component('miaosha', {
    ...
    template: '<li :style="' + "'background-color:'" + '+bg_color+' + "';'" + '" class="ui-draggable" title="点击进行修改,拖动交换位置.">' +
        '<div v-on:click="del_left(index)" class="deleteButton"></div>' +
        '<div v-on:click="onc_banner(index,$event)" :class="\'cubeNavigationArea column-\'+style_num+\' clearfix\'">' +
        '<div class="secAtyIngList secAtyList" v-for="d in list">' +

        '<div class="secGoodsItem"> <div class="countdownWrap">' +
        '<span style="color:#444;font-size:14px;">距结束</span> ' +
        '<div class="countdown"><span class="cdDay">00</span>:<span class="cdHours">00</span>:<span class="cdMinute">00</span>:<span class="cdSecond">00</span></div>' +
        '</div> <div class="secGoodsInfo"> <div class="secLeft"> <img :src="d.imgurl" class="commGoodsImg"> </div> ' +
        '<div class="secRight"> <span class="secGoodsName" :style="' + "'font-size:'" + '+font_size+' + "'px;color:'" + '+text_color+' + "';'" + '">{{d.name}}</span> ' +
        '<div> <span style="color:#666;font-size:14px;">秒杀价:</span> <span style="color:#FF4A05;">￥</span>' +
        '<span class="secPrice">{{d.price}}</span> <span class="originalPrice">￥{{d.oprice}}</span> </div> ' +
        '<div class="rBottom"> <div> <div class="barWrap"> <div class="bar" :style="\'width:\'+d.progress+\'%;\'"></div></div>' +
        '<span class="leftNum">剩余：199</span>' +
        '</div><div class="secBtn" >立即秒杀</div></div> </div> </div> </div></div></div></li>',
...
//砍价
Vue.component('kanjia', {
    props: ['list', 'index', 'bg_color', 'font_size', 'text_color'],
    template: '<li :style="' + "'background-color:'" + '+bg_color+' + "';'" + '" class="ui-draggable" title="点击进行修改,拖动交换位置.">' +
        '<div v-on:click="del_left(index)" class="deleteButton"></div>' +
        '<div v-on:click="onc_banner(index,$event)" class="cubeNavigationArea clearfix">' +
        '<div class="index_info_li" v-for="(v,k) in list" :style="\'background:\'+bg_color+\'\'"> ' +
        '<div class="index_info_pic" :style="\'background:url(\'+v.imgurl+\') no-repeat center center;background-size: cover;\'"> ' +
        '<div class="index_info_count"> <span>剩余100份</span> </div> </div> ' +
        '<div class="index_info_tit" :style="\'color:\'+text_color+\';width:300px;height:30px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;\'">' +
        '{{v.name}}</div> <div class="index_info_uface clear"> ' +
        '<div class="image" style="background:url(https://wx.qlogo.cn/mmopen/vi_32/TT3L7ZtJ64xEMGzTBwKicHorib9YHQxt3qaWqfesT0Syx07oU6jiaP1v9YHpESC4s9g4wuG85FyfOoMZpMicBHrlbw/132) no-repeat center center;background-size: cover;"></div> ' +
        '<div class="image" style="background:url(https://wx.qlogo.cn/mmopen/vi_32/TT3L7ZtJ64xEMGzTBwKicHorib9YHQxt3qaWqfesT0Syx07oU6jiaP1v9YHpESC4s9g4wuG85FyfOoMZpMicBHrlbw/132) no-repeat center center;background-size: cover;"></div>' +
        ' <div class="image" style="background:url(http://ddfwz.sssvip.net/asmce/kanjia/index_more.jpg) no-repeat center center;background-size: cover;"></div> ' +
        '<div class="user_no">2人正在参加</div> </div> <div class="index_price clear"> <div class="price_left"> ' +
        '<div class="price02">￥<span>{{v.gprice}}</span><span class="price01"> 原价￥{{v.price}}</span> </div> </div> ' +
        '<div class="price_right" style="padding-left: 190px;margin-top: -30px;padding-bottom: 20px;"> <div class="buy_btn">去砍价</div> </div> <div class="clear"></div></div></div>' +
        '</div></li>',
    methods: {
        //删除
        del_left: function (index) {
        ...
        //选中
        onc_banner: function (index, e) {
            vm_choose_item(index, 'kanjia', true);
        },
    }
});
//拼团
Vue.component('pintuan', {
    props: ['list', 'index', 'bg_color', 'font_size', 'text_color'],
    template: '<li :style="' + "'background-color:'" + '+bg_color+' + "';'" + '" class="ui-draggable" title="点击进行修改,拖动交换位置.">' +
        '<div v-on:click="del_left(index)" class="deleteButton"></div>' +
        '<div v-on:click="onc_banner(index,$event)" class="cubeNavigationArea clearfix">' +
        '<div class="goods-group" style="margin-top:8px;margin-bottom: 8px;"> ' +
        '<div class="goods-item pull-left" v-for="(v,k) in list" :style="\'background:\'+bg_color+\'\'">' +
        ' <img :src="v.imgurl"/><div class="goods-title" :style="\'color:\'+text_color+\'\'">{{v.name}}</div> ' +
        '<div class="pull-left kaituan"> <div class="pull-left" style="margin-top: 10px;">' +
        ' <span class="goods-price">¥{{v.gprice}}</span> <span class="goods-mprice" style="font-size: 14px;">¥{{v.price}}</span> <span class="user_no" style="font-size: 14px;">2人团</span> </div> <div class="pull-right" style="margin-right: 4px;"> <div class="btn span-center" style="border:0;font-size:15px;line-height: 30px">去开团 </div> </div> </div> </div> <div class="clearfix"></div></div>' +
        '</div></li>',
    methods: {
        //删除
        del_left: function (index) {
        ...
        //选中
        onc_banner: function (index, e) {
            vm_choose_item(index, 'pintuan', true);
        }
    }
});
//广告位
...
//商品集<em style="padding-left: 5px;font-size: 10px;color: #68838B">{{g.description}}</em>
Vue.component('goods', {
    props: ['list', 'index', 'bg_color', 'font_size', 'text_color', 'style_type', 'add_type', 'add_sort', 'add_num', 'add_cate'],
    template: '<li title="点击进行修改,拖动交换位置." :style="\'background-color:\'+bg_color" class="ui-draggable"><div v-on:click="del_left(index)" class="deleteButton"></div><div  v-on:click="onc_banner(index,$event)" :class="\'cubeNavigationArea goods_column-\'+style_type+\' clearfix\'"><div v-for="g in list" class="cubeLink cubeLink1 Ggdh" style="border-bottom: 1px solid rgb(229, 229, 229); border-right: 1px solid rgb(229, 229, 229);">' +
        ...
        '<div class="cubeLink_text g_link" style="position: relative !important;"><div style="display:flex;flex-direction:column;"><em :style="' + "'color:'" + '+text_color+' + "';font-size:'" + '+font_size+' + "'px;font-weight: bold; padding-left: 5px;'" + '">{{g.name}}</em></div><p class="cubeLink_subText_p">￥{{g.price}}</p></div></div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            vm_del_item(index);
...
Vue.component('position', {
    props: ['index', 'name','business_name', 'show_type', 'bg_color', 'text_color'],
    template: '<li class="ui-draggable" :style="\'background:\'+bg_color+\';\'" title="点击进行修改,拖动交换位置.">' +
        '<div v-on:click="del_left(index)" class="deleteButton"></div><div v-on:click="onc_banner(index)" class="position">' +
        '<img v-if="show_type==2" src="' + host + 'addons/yb_shop/core/public/menu/images/pos.png"> <div v-if="show_type==1" class="wrap" :style="\'color:\'+text_color+\';\'"> ' +
        '<em class="fr iconfont icon-arrow-right"></em> <span class="title"><i class="Hui-iconfont Hui-iconfont-weizhi" :style="\'color:\'+text_color+\';\'"></i>{{name}}</span> ' +
        '</div> </div></li>',
    methods: {
        //删除
        del_left: function (index) {
...
Vue.component('edit_piclist', {
    ...
    computed: {
        html_style: function () {
            return this.style_type == 2 ? 'text-align:center;position:absolute;left:0;bottom:-3px;width:100%;line-height:29px;background-color:rgba(0,0,0,.5);text-decoration:inherit;' : '';
        }
    },
})
//悬浮客服
