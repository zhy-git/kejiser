	...
	function __construct(){
		...
		if(!empty($mch_id))
		{
			...
			if(!empty($GLOBALS['key']) && $GLOBALS['key'] == 'bargain'){
				...
				$ms_url ="https://" . $host.$child_path[0] .'addons/yb_shop/core/ms_pay.php';
				$this->APP_URL = $param['APP_URL_MS']?$param['APP_URL_MS']:$ms_url;
			}elseif(!empty($GLOBALS['key']) && $GLOBALS['key'] == 'reserve'){
                if (isset($_SERVER['HTTP_X_REAL_HOST'])) {
                    $host = $_SERVER['HTTP_X_REAL_HOST'];
                } else {
                    $host = $_SERVER['HTTP_HOST'];
                ...
                $reserve_url ="https://" . $host.$child_path[0] .'addons/yb_shop/core/reserve_pay.php';
                $this->APP_URL = $param['APP_URL_RESERVE']?$param['APP_URL_RESERVE']:$reserve_url;
            }else{
