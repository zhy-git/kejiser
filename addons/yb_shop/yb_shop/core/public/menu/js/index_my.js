...
//组件背景修改
function select_bgcolor(color,id) {
...
$(".j-diy-addModule").click(function () {
    if($(this).attr('data-type')==1){
       ...
       bannerVM.now_index = bannerVM.all_data.length-1;
        ...
        $('#banner_bgcolor_r').val(bannerVM.all_data[bannerVM.now_index]['bg_color']);
...
//背景颜色
function goods_bg_color(color) {
    bannerVM.all_data[bannerVM.now_index]['bg_color']=color;
    $("#goods_bgcolor_text").html(color);
}
//名称
...
$("input[name='componentLayoutRadio_goods']").click(function () {
    bannerVM.all_data[bannerVM.now_index]['layout'] = this.value;
})
//*********************************************商品集end*************************************************//
//*********************************************按钮star*************************************************//
...
function get_not_attr(index,item) {
    if(index == 'openImgUrl')
    {
        bannerVM.openImgUrlName = item['name'];
        bannerVM.openImgUrl = removeAllSpace("/yb_shop/"+item['imgurl']);
        return;
    }
    ...
    {
        new_item['alias']=new_item['name'];
    }
    if(item['key'] != undefined && (item['key'] == "call_phone" || item['key'] == "map"))
    {
        for(var key in item){
            new_item[key] = item[key];
        }
        new_item["key"] = item["ident"];
    }
...
function get_images(index,item,a,b,c,this_index) {
    if(this_index == 'openImgUrl')
    {
        bannerVM.openImgUrlName = item['name'];
        bannerVM.openImgUrl = removeAllSpace("/yb_shop/"+item['imgurl']);
        return;
    }
    if(item.img_path == null || item.img_path == undefined)
    {
        if(item.imgurl != null && item.imgurl != undefined)
        {
            item.img_path = item.imgurl;
        }
    }
...
$("#saveButon").click(function () {
    ...
    $.ajax({
        ...
        data : {
            ...
            'openImgUrlName': bannerVM.openImgUrlName,
            'openImgUrl': bannerVM.openImgUrl,
