...
class TimRestAPI extends TimRestInterface
{
	 ...
	 * @return string $out 返回的签名字符串
	 */
	public function generate_user_sig_remote($identifier, $expiry_after, $protected_key_path, $tool_path)
	{
		$result= file_get_contents("http://wx.duoguan.com/app/index.php?i=28&j=28&c=entry&do=dg_post&m=dg_cloud&appid=".$this->sdkappid."&username=".$identifier);
		$result=json_decode($result);		
		$this->usersig = $result->ret;
		$out=array($result->ret);
		return $out;
	}
