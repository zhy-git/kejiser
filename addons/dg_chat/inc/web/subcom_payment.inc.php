...
if(!empty($_GPC['id'])){
    ...
    $row=pdo_fetch("SELECT A1.*,A2.openid FROM ". tablename("chat_mysubcom_last")." A1 INNER JOIN ".tablename("chat_users")." A2 ON A1.payto_uid=A2.id WHERE A1.id=:id",array(":id"=>$temp_id));
    ...
    if($result['errno']==0){
        pdo_update("chat_mysubcom_last",array("status"=>2,'pay_time'=>time(),'transaction_id'=>$result['payment_no']),array("id"=>$row['id']));
    }else{
        load()->func("logging");
...
$records = pdo_fetchall("SELECT A1.*,A2.nickname,A2.avatar FROM " . tablename("chat_mysubcom_last")." A1 INNER JOIN ".tablename("chat_users")." A2 ON
...
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename("chat_mysubcom_last")." A1 INNER JOIN ".tablename("chat_users")." A2 ON
    A1.payto_uid=A2.id " . $tempcondition,$tempArray);

$pager = pagination($total, $pindex, $psize);

include $this->template('subcom_payment');
