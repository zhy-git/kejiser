...
$tempcondition=" WHERE A1.uniacid = '{$_W['uniacid']}' AND A1.is_del is null and A1.series_id=".$series_id;
...
$records = pdo_fetchall("SELECT A1.*,A2.room_name,A2.create_nickname FROM ".tablename("chat_topic")." A1 INNER JOIN 
    ".tablename("chat_room")." A2 ON A1.room_id=A2.room_id  ".  $tempcondition." ORDER BY gl_order DESC,topic_order DESC,A1.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,$tempArray);
...
include $this->template('series_topic');
