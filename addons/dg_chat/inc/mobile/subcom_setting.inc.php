...
$subcom_percent=$chat_room['subcom_percent']*100;

/*分成比例存储开始*/
if(!empty($_GPC['submit'])&&$is_manager){
    header('content-type:application/json;charset=utf8');
    $percent=intval($_GPC['percent']);
    $check_topic=pdo_fetchcolumn("SELECT COUNT(0) FROM ".tablename("chat_topic")." WHERE room_id=:room_id AND end_time=0 and is_del is null ",array(":room_id"=>$room_id));

    if($check_topic>0&&$chat_room['subcom_status']==1&&$percent!=$subcom_percent){
        $fmdata = array(
            "success" => -1,
...
include $this->template('subcom_setting');