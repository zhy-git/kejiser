...
lilv_array[2][1][5] = 0.0475;//商贷 1～5年 4.9%
lilv_array[2][1][10] = 0.0490;//商贷 5-30年 5.05%
...
function QuickCount_MakeDown(rate)
{
   if(rate == 'undefined' || rate =='') rate = 30;
   var total_money = $gid('E_total_money').value;
   total_money = parseInt(total_money);
   var first_money  = total_money * rate ;
   first_money = changeTwoDecimal_f(first_money);
   //===首付款
   $gid('divDown').innerHTML= first_money;
}

function QuickCount_MakeMonthMoney(rate)
{
     if(rate == 'undefined' || rate =='') rate = 30;
       var total_money = $gid('E_total_money').value;
     total_money = parseInt(total_money);
	 var first_rate = $gid('first_rate').value;
	   if(first_rate=='undefined' || first_rate=='') first_rate = 0.3;
	 var dk_money = (total_money - (total_money * first_rate))*10000 ;
     var total_month = rate * 12;
	 var lilv = getlilv(2, 1, rate);//得到利率 商业贷款
	 var month_money1 = getMonthMoney1(lilv, dk_money, total_month);//调用函数计算
	  month_money1 = Math.round(month_money1 ) ;
	 $gid('divMonthMoney1').innerHTML = month_money1;
	 $gid('divMonthMoney').innerHTML = month_money1;
}
//=========================
function ext_dk_totalmoney(){
		//1.本金还款
			//月还款
			var all_total2 = 0;
			var month_money2 = "";
...
//====================
function cal_jun_price(total, area){
   var area_price = 0;
    if(total && area){
         total = total  *10000;
    	 area_price = Math.round(total/area);
    }else{
    	area_price = 0;
    }
    $gid("junjia").innerHTML = "<font style=color:red > "+ area_price +"</font>元/平方米";
}
//=========================================
function changeDIV(id){
	if(id == "1"){
		$gid('title01').className = "maintitletd1";
		$gid('title02').className = "maintitletda font01";
		$gid('title03').className = "maintitletdb font01";
		$gid('title04').className = "maintitletda font01";
		$gid('title05').className = "maintitletd font01";
	}else if(id == "2"){
		$gid('title01').className = "maintitletd font01";
		$gid('title02').className = "maintitletda1";
		$gid('title03').className = "maintitletdb font01";
		$gid('title04').className = "maintitletda font01";
		$gid('title05').className = "maintitletd font01";
	}else if(id == "3"){
		$gid('title01').className = "maintitletd font01";
		$gid('title02').className = "maintitletda font01";
		$gid('title03').className = "maintitletdb1";
		$gid('title04').className = "maintitletda font01";
		$gid('title05').className = "maintitletd font01";
	}else if(id == "4"){
		$gid('title01').className = "maintitletd font01";
		$gid('title02').className = "maintitletda font01";
		$gid('title03').className = "maintitletdb font01";
		$gid('title04').className = "maintitletda1";
		$gid('title05').className = "maintitletd font01";
	}else if(id == "5"){
		$gid('title01').className = "maintitletd font01";
		$gid('title02').className = "maintitletda font01";
		$gid('title03').className = "maintitletdb font01";
		$gid('title04').className = "maintitletda font01";
		$gid('title05').className = "maintitletd1";
	}
	for(var i = 1; i < 6; i++){
		if($gid('calculate0' + i) != null){
			$gid('calculate0' + i).style.display = "none";
		}
		if($gid('note0' + i) != null){
			$gid('note0' + i).style.display = "none";
		}
	}
	$gid('calculate0' + id).style.display = "block";
	$gid('note0' + id).style.display = "block";
}
...
function ShowLilv(month, lt){
	...
	$gid("sdlv").innerHTML = myround(lilv_array[lt][1][year] * 100, 2) + "%";
	$gid("gjlv").innerHTML = myround(lilv_array[lt][2][year] * 100, 2);
}
...
function chanage_type_mm(){
	if($gid("dengeben2").checked)
		$gid("type1_mm1").innerHTML = '&nbsp;&nbsp;月均金额：<textarea name="month_money1" rows="5" ></textarea>';
	else
		$gid("type1_mm1").innerHTML = '&nbsp;&nbsp;月均还款：<input name="month_money1" type="text" class="guestbook02" />元';
}
...
function pl(){
	...
	XmlHttp.open("get", "/house/web/newhouse_calculateTool_projList.php?s="+Math.random()+"&city="+encodeURIComponent($gid('abcity').value)+"&price=" + Math.round(document.myform.rg08.value), false);
	XmlHttp.send(null);
	if(XmlHttp.status == 200){
		$gid('projList').innerHTML = XmlHttp.responseText;
	}
}
function closeCityList() {
	if($gid('searchExpert2').style.display!="none") $gid('searchExpert2').style.display='none';
}
function ChangeCity(city) {$gid('abcity').value=city;$gid('showcity').innerHTML=city;closeCityList()}