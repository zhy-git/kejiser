...
<div class="tr">
	<a href="javascript:;" class="add_form_btn topbar_jsbtn" js="addguysort">添加文章分类</a>
</div>
<?php  if(!empty( $list )) { ?>
	   ...
	   <?php  if(is_array($list)) { foreach($list as $item) { ?>
	    	<tr> 
			    ...
			    <td class="table_cell oper last_child tr opclass" style="position: relative;">
			    	<a href="<?php  echo self::pwUrl('site','article',array('op'=>'list','artsortid'=>$item['id']))?>">查看分类文章</a>
			    	<!-- <a href="javascript:;" class="copy_url" data-href="<?php echo '/zofui_sitetemp/pages/artlist/artlist?sid='.$item['id']?>">复制路径</a> -->
			    	<a  href="javascript:;" class="edit_listitem" id="<?php  echo $item['id'];?>">编辑</a>
			    	<a href="<?php  echo self::pwUrl('site','artsort',array('op'=>'delete','id'=>$item['id']))?>" onclick="return confirm('删除不能恢复，确定要删除吗？');">删除</a>
			    </td>
	    	</tr>
	    <?php  } } ?>
   	</tbody>
				...
				<div class="frm_control_group">
					...
					<div class="frm_controls msg">
						...
						<span>
							...
							<select style="width: 200px;padding-left: 10px;" name="artid">
								<?php  if(is_array($artinfo)) { foreach($artinfo as $item) { ?>
...
<script type="text/javascript">
	$(function(){
		...
		$('.edit_listitem').click(function(){
			...
			Http('post','json','findartsort',{fid:nowfid},function(data){
				if(data.status == 200){
		...
		$('#confirm_addform').click(function(){
			var postdata = {
				fid : fid,
				artid : $('select[name=artid]').val(),
				name : $('input[name=name]').val(),
				number : $('input[name=number]').val(),
			};
			Http('post','json','addartsort',postdata,function(data){
				if(data.status == 200){
					webAlert(data.res);
					setTimeout(function(){
						location.href = '';
