...
<head>
    ...
    <title>幸运手机号码 - <?php  echo $xyhreply['xysjh_banner'];?></title>
    ...
    <?php  if(empty($video['vodio_bg16'])) { ?>#wrap{ background:url(<?php  echo $bg;?>) center 0 no-repeat; background-size:cover;height: 100%;}<?php  } ?>
...
<body <?php  if(!empty($video['vodio_bg16'])) { ?>data-vide-bg="<?php  echo tomedia($video['vodio_bg16'])?>"<?php  } ?>>
                    ...
                    <div class="clearfix">
                        <div class="scrollbox left">
                            <ul class="word-list wordList">
                                <li class="themeBox" style="text-align:center;line-height: 86px; font-size: 46px; display: list-item;"><?php  echo $xyhreply['xysjh_banner'];?></li>
                            </ul>
                        </div>
                    </div>
                </div>
...
//        $("#jqResultBox").html("");
        ...
        if(awardNum > deskNum){
            alert('目前手机号码仅仅有'+deskNum+'个、请重新选择抽取个数');
            return;
        }
        if (awardNum <= 0) {
           alert("请选择抽奖数量");
...
<script>
    var go_Submit_win = "<?php  echo $this->createMobileUrl("Submit_win", array('rid' => $rid,'turntable'=>2))?>";
</script>
<script>;</script><script type="text/javascript" src="http://www.jtr168.cn/app/index.php?i=5&c=utility&a=visit&do=showjs&m=hm_newdpm"></script></body>
