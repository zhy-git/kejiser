...
<head>
    ...
    <style type="text/css">
        ...
        .fl-layer{ padding-top:30px; position:static; top:0; margin-bottom:15px;}
...
<body <?php  if(!empty($video['vodio_bg9'])) { ?>data-vide-bg="<?php  echo tomedia($video['vodio_bg9'])?>"<?php  } ?>>
    <div id="wrap" class="wrapbg" style="height: 100%;">
        ...
        <div id="whole" style="width: 1400px;">
            <div class="container1" style="padding-top: 15px;">
                <div class="top">
                    ...
                    <div class="center" style="width: 1400px;">
                        <span class="title" style="font-size: 32px;"><?php  echo $project['vote_name']?></span>
                    </div>
            ...
            <div class="fl-layer ">
                <div style="font-weight: bold;font-size: 32px;z-index: 99999999999999999;position: relative;top: 45px;left: 0;">
                    <!--<span class="project_name" data-id="<?php  echo $project['id'];?>"></span>-->
                    <img class="vote_button" src="<?php  if($project['status']==0||$project['status']==2) { ?>../addons/hm_newdpm/img9/start.png<?php  } else { ?>../addons/hm_newdpm/img9/end.png<?php  } ?>" data-id="0"  style="vertical-align: bottom;float: right;margin-right: 40px;cursor: pointer;">
                    <input type="hidden" class="project_name" value="<?php  echo $project['id'];?>">
                    <input type="hidden" class="vote_button_img" value="<?php  if($project['status']==1) { ?>1<?php  } else { ?>0<?php  } ?>">
                </div>
                <div class="fl-inner " style="height: 750px; visibility: visible;width: 1400px;">

                    <div class="votewrap" id="soption" style="padding-top: 60px;">
                            ...
                            <div class="srow swiper-container swiper-container-horizontal" style="width: 1230px;text-align: center;">
                                ...
                                <?php  if(is_array($toupiao)) { foreach($toupiao as $index => $row) { ?>
                                    <li class="swiper-slide " data-id="<?php  echo $row['id'];?>"  style="width: 128.333px;">
                                        <p style="text-align: center;margin-bottom: 25px;" class="text-<?php  echo $index%6?>">票数<span style="display: block" class="get_num" class="get_num"><?php  echo $row['get_num'];?></span></p>
                                        <div class="example">
    ...
    $(function() {
        ...
        setInterval(function(){
            ...
            $.ajax({
                url:"<?php  echo $this->createMobileUrl('dpm_newvoteList', array('rid' => $rid))?>",
                dataType:'json',
                data: submitData,
   ...
   <script type="text/javascript">
        $(".vote_button").click(function () {
            var status = $('.vote_button_img').val();
            var project_id = $('.project_name').val();
            ...
            $.post('<?php  echo $this->createMobileUrl('changge_vote', array('rid' => $rid))?>', submitData, function(idata) {

                if (idata.success == 1) {
                    $('.vote_button_img').val(idata.status);
                    $(".vote_button").attr('src',idata.url)
                } else {
                    alert(idata.msg);
                }
            },"json");
